"""
Pydantic models for Knowledge Service REST API.

Defines request/response models for all operations:

Read path (5 operations):
- resolve-context: Given a repo, return operational pages for the scope chain
...
class ScopeFilter(BaseModel):
    """
    Optional filter for scoping queries.
...
class PageSummary(BaseModel):
    ...
    type: str = Field(..., description="Page type: service-overview, repo-profile, procedure, guide, concept, team-conventions, keystone, learning")
    mode: str = Field(..., description="Mode: reference, operational, or keystone")
    scope: dict = Field(default_factory=dict, description="Scope dict with optional keys: company, org, squad, service, repo")
...
class PageFull(PageSummary):
    ...
    applies_to: list = Field(default_factory=list, description="Cross-cutting: repos/services this applies to")
...
class ResolveContextRequest(BaseModel):
    """
    Request to resolve the full scope chain for a repo and return operational pages.
    """
...
class ResolveContextResponse(BaseModel):
    """
    Response containing the entry point page, operational pages, and resolved scope chain.
    """
    entry_point: Optional[PageSummary] = Field(None, description="The repo-profile page for the given repo")
    operational_pages: list[PageSummary | PageFull] = Field(default_factory=list, description="All operational pages applicable to the scope chain")
    scope_chain: dict = Field(default_factory=dict, description="Resolved scope hierarchy: repo, service, squad, org, company")
...
class SuggestMetadataRequest(BaseModel):
    ...
    hints: Optional[dict] = Field(None, description="Optional partial knowledge, e.g. {'scope.service': 'Document Service'}")
...
# (Renumbered: was Operation 8 before check-duplicates was added)
# ============================================================================
...
class RegistryEntryModel(BaseModel):
    ...
    scope_org: Optional[str] = Field(None, description="Org mapping (services/teams only)")
