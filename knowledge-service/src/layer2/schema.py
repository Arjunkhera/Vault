...
@dataclass
class RegistryEntry:
    """A single entry in a registry (tag, service, team, or org)."""
    id: str
    description: str = ""
    aliases: list[str] = field(default_factory=list)
    scope_org: Optional[str] = None  # services/teams only
...
class SchemaLoader:
        ...
        return {
            ...
            "registries": {
                name: [
                    ...
                    | ({"scope_org": e.scope_org} if e.scope_org else {})
                    for e in entries
                ]
    ...
    def _load_registries(self) -> None:
        ...
        for reg_file in sorted(reg_dir.glob("*.yaml")):
            name = reg_file.stem  # e.g. "tags", "services"
...
class PageValidator:
    ...
    def validate(self, metadata: dict) -> ValidationResult:
        ...
        4. registry-backed field checks (tags, scope.org, scope.squad, scope.service)
        ...
        if isinstance(scope, dict):
            self._check_registry_scalar(scope, "org", "orgs", "scope.org", errors, warnings)
            self._check_registry_scalar(scope, "squad", "teams", "scope.squad", errors, warnings)
            self._check_registry_scalar(scope, "service", "services", "scope.service", errors, warnings)
