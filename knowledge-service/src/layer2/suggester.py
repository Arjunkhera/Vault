...
class ScopeSuggestions:
    ...
    program: Optional[FieldSuggestion] = None
    repo: Optional[FieldSuggestion] = None

    def to_dict(self) -> dict:
        d: dict = {}
        for name in ("program", "repo"):
...
class MetadataSuggester:
    ...
    Usage:
        ...
        result = suggester.suggest(content, hints={"scope.program": "anvil-forge-vault"})
    """
    ...
    def suggest(self, content: str, hints: Optional[dict] = None) -> SuggestionResult:
        ...
        Args:
            ...
            hints:   Optional dict of known values (e.g. {"scope.program": "anvil-forge-vault"}).
        """
        hints = hints or {}
    ...
    def _suggest_scope(self, body: str, kb_status: str, hints: dict) -> ScopeSuggestions:
        ...
        # --- program (from hint or derived from related repo profiles) ---
        program_value: Optional[str] = None

        if hints.get("scope.program"):
            program_value = hints["scope.program"]
            scope.program = FieldSuggestion(
                value=program_value, confidence="high", reason="From hint"
            )
        elif self._store and kb_status == "populated":
            # Try to find related pages (by repo mention) and extract their program
            repo_mentions = _extract_repo_mentions(body)
            if repo_mentions:
                # Search for pages mentioning the same repo
                try:
                    for repo in repo_mentions[:3]:  # check first 3 repo mentions
                        results = self._store.hybrid_search(repo, limit=5)
                        for result in results:
                            doc_content = self._store.get_document(result.file_path)
                            if doc_content:
                                related = parse_page(doc_content)
                                if related.type == "repo-profile" and related.scope.get("repo") == repo:
                                    if related.scope.get("program"):
                                        program_value = related.scope.get("program")
                                        scope.program = FieldSuggestion(
                                            value=program_value,
                                            confidence="high",
                                            reason=f"Derived from repo-profile for '{repo}' → program mapping",
                                        )
                                        break
                        if program_value:
                            break
                except Exception as e:
                    logger.debug("Program suggestion KB search failed: %s", e)

        if scope.program is None:
            scope.program = FieldSuggestion(
                value=None,
                confidence="none",
                reason="No program info found in content or related pages",
            )
